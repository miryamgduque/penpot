...
1. [Phase 01 — Backend: skill state](./todo-phase-01-backend-skill-state.md) — `profile_skill_state` table (per-user, `file_id` NULL = account / set = per-file) + RPC to get/set `enabled`, mirroring ai-providers.
2. [Phase 02 — Frontend state + resolution](./todo-phase-02-frontend-state-resolution.md) — fetch/persist account + per-file state, effective-enabled resolution feeding the router.
