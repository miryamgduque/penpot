...
- [x] Read `backend/src/app/rpc/commands/ai_providers.clj` (table, upsert, command shape) as the template
- [x] Find the migrations dir + registration (`app.migrations`) and the sql helpers (`app.db`)
- [x] Confirm how commands are registered/exposed and how the frontend `rp/cmd!` names map to them
- [x] Decide the exact columns (booleans vs a status enum) and defaults

## Checklist

- [x] Migration: `profile_skill_state (profile_id, skill, file_id NULL, enabled, modified_at)`,
...
- [x] `app.rpc.commands.skill-state`: `get-skill-states {?file-id}` (account rows + this file's rows),
      `set-skill-enabled {skill enabled ?file-id}` (NULL file-id = account default) — upsert on
      conflict `(profile_id, skill, file_id)`
- [x] Register the commands; add malli/spec schemas for params
- [x] Only persist **non-default** rows where sensible (avoid a row per built-in skill by default)
- [x] Backend tests for the commands (set → get round-trip, upsert overwrite)
- [x] `make lint/backend` + backend tests pass
...
- [x] Note the exact command names + row shape for Phase 02 to consume
