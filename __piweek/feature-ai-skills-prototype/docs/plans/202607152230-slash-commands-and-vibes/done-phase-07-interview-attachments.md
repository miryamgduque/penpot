...
- [x] Phases 02/03/05 are done (schema, form UI, skill body all get touched)

## Checklist

- [x] Schema: `allow_images` (boolean) on `text` questions; tool description
      explains the images arrive attached to the tool result, in question
      order
- [x] `elicitation.cljs`: `:images` joins the per-question ui state; an
      `attachments` fn collects `{qid count}` + the flat image vector in
      form order; caps (3 per question, 5 per form — mirrors the composer)
- [x] Form UI: attach button + thumbnails on `allow_images` questions,
      reusing the composer's `read-images` pipeline (recompress to WebP,
      1568px long edge); remove per thumbnail
- [x] Submit path: `submit-form` carries the images; `submit-pending-form!`
      ...
      turn-loop changes
- [x] The answers-summary bubble shows the attached thumbnails
      (`append-message` already renders `:images` on user bubbles) —
      `submit-form` gains the images argument
- [x] Text-only models: the existing `strip-images`/“images omitted” note
      already covers tool-result images — verify, don't rebuild
- [x] Vibes skill body: differentiation question set
