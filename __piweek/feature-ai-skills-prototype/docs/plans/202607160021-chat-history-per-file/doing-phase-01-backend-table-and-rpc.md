...
- [ ] Verify plan is still valid (no conflicts with other plans/sessions — Miryam works in the same tree)
- [ ] Check the latest migration number is still `0156` (`ls backend/src/app/migrations/sql/ | tail`); renumber if she added one
- [ ] Re-read `backend/src/app/rpc/commands/skill_state.clj` and `ai_providers.clj` for the current house patterns (ON CONFLICT upsert, schema style, `::doc/added "2.13"`)
- [ ] Confirm `db/tjson` + `db/decode-transit-pgobject` signatures in `backend/src/app/db.clj`

## Checklist

- [ ] Migration `backend/src/app/migrations/sql/0157-add-profile-agent-chat-table.sql`:
...
- [ ] Register in `backend/src/app/migrations.clj`
- [ ] `backend/src/app/rpc/commands/agent_chats.clj`:
...
- [ ] Register the namespace in `backend/src/app/rpc.clj` `scan-ns`
- [ ] Lint: clj-kondo + cljfmt clean (run in devenv against the worktree path, `/opt/utils/bin/cljfmt`)
- [ ] Commit in the worktree: `:sparkles: Add per-file agent chat storage` (no approval pause — the merge in Phase 04 is the gate)
...
- `backend/src/app/migrations/sql/0157-add-profile-agent-chat-table.sql` — new table
