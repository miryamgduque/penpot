...
1. [Phase 01 — Backend: `team_skill_seen` + `arrived?` + mark-seen RPC](./doing-phase-01-backend-arrival-state.md) — new table/migration, extend `get-team-skills` with a computed `:arrived?`, add `::mark-team-skill-seen` mutation.
2. [Phase 02 — Frontend data: surface `arrived?` through the catalog](./todo-phase-02-frontend-data-arrival.md) — thread the field through `team_skills.cljs` → `agent_skills.cljs`'s `full-catalog`/`team-skill->entry`, add the `mark-team-skill-seen!` action.
