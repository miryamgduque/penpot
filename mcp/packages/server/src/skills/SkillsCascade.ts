...
import aikitSkills from "./aikit-skills.json";

/**
 * Stub skills for the broader scopes (kept in sync with
 * ai-skills/src/skills/builtin.ts). The platform scope additionally carries
 * the official penpot-ai-kit set, generated by
 * ai-skills/scripts/import-aikit.mjs into aikit-skills.json.
 */
export const BUILTIN_SKILL_SOURCES: { scope: Scope; source: string }[] = [
    ...(aikitSkills as string[]).map((source) => ({ scope: "platform" as Scope, source })),
    {
        scope: "platform",
        source: `---
name: a11y-contrast
